...
function SettingsContent() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/coach/profile?email=${encodeURIComponent(email)}`)
  ...
  const uploadPhotoToAPI = async (base64Data: string, filename: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/coach/profile/photo`, {
        method: 'POST',
        headers: {
  ...
  const handleDisconnectGoogleCalendar = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/coach/google-calendar/disconnect`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const checkGoogleCalendarStatus = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/coach/google-calendar/status?coach_email=${encodeURIComponent(email)}`)
  ...
  // Check Google Calendar status on load
  useEffect(() => {
