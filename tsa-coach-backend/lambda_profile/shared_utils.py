...
def get_table_name(table_type: str) -> str:
    ...
    env_var = table_env_map.get(table_type)
    if not env_var:
        raise ValueError(f"Unknown table type: {table_type}")
    
    table_name = os.environ.get(env_var)
    if not table_name:
        raise ValueError(f"Environment variable {env_var} not set for table type {table_type}")
    
    return table_name


def standardize_error_response(error: Exception, status_code: int = 500) -> Dict[str, Any]:
